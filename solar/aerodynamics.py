...
def form_factor_ellipsoid(aspect_ratio):
    """
    Estimates the form factor of an ellipsoid (ratio of true drag to drag based on turbulent flat plate on wetted area)
    Assumes ellipsoid is at zero angle of attack.
    VERY heuristic guess for 1 < aspect_ratio < Inf; this model is dubious at best.
    :param aspect_ratio: length / diameter of ellipsoid
    :return: Form factor
    """
    ### Notes
    # For AR = Inf, form factor should be 1 (flat plate)
    # For AR = 1, this should correspond to sphere data.
    #   CD_sphere ~ 0.2 at relevant (supercritical) Reynolds = 1e7, referenced to XSEC AREA.
    #   This is 0.1 refernced to wetted area (ratio = 2). Predicted: 0.00277 @ Re = 1e7; form factor of 36.1.
    # Found a chart on google that showed drag for a 2:1 ellipse was roughly half that of a sphere at relevant Re.

    # return 1 + 36.1 / aspect_ratio # Based on three sketchy data points and literally a made-up curve, this is extremely dubious, but it's a zeroth-order guess.
    return 1 + 1 * (
        1.5 / aspect_ratio ** 1.5 + 7 / aspect_ratio ** 3
    ) # Based on Hoerner, qtd. in Schaufele
